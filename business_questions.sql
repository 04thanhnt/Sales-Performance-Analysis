...
--3/Average price of products,number of orders in each product category during the period of 2023-2025
select productcategory, avg(price) average_price, count(orderid) as no_of_orders
from vw_full_orders
group by productcategory
order by avg(price) desc

--4/ Orders in each year per region, with revenue and gross profit calculated
select year(orderdate) as order_year, region, 
...
select order_year, region,productname, productcategory, total_quantity
from (
...
select order_year, region, customerid, no_of_products, total_revenue
from (
		select year(orderdate) as order_year, 
			   region, 
			   customerid,
...
select top 10 customerid, sum(revenue) as total_revenue , sum(quantity) as total_quantity
from vw_full_orders
group by customerid
order  by sum(revenue) desc
...
--13/Pareto analysis by products
